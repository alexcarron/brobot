...
const Parameters = {
	...
	Contestant: new Parameter({
		...
		description: "The eliminated contestant to be included in the alliance",
		isRequired: true,
	}),
}

module.exports = new SlashCommand({
	name: "create-eliminated-alliance",
	description: "Create an alliance with a given eliminated contestant",
	...
	execute: async function execute(interaction) {
		...
		for (const index in contestantIDs) {
			...
			const isContestant = await memberHasRole(member, ids.sandSeason3.roles.eliminated, true);

			if (!isContestant) {
				editReplyToInteraction(interaction,
					`User <@${contestantID}> is not a valid eliminated contestant. Please provide valid contestants.`
				)
				return;
      }
		}
