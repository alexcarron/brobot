...
command.execute = async function execute(interaction, args, isTest=false) {
	if (interaction) {
		try {
			await interaction.reply({content: "Starting Trial Results...", ephemeral: true});
		}
		catch {
	...
	staff_chnl = await getChannel((await global.Game.getGuild()), channel_ids.staff);
	...
	const killPlayer = async function(victim_name, killer_name="Host", type="lynch") {
		...
		if (type == "lynch") {
			if (global.Game.Players.get(victim_name).role == "Fool") {
				let fool_chnl = await getChannel((await global.Game.getGuild()), global.Game.Players.get(victim_name).channel_id);
			...
			for (let exe of executioners) {
				...
				if ( exe.exe_target == victim_name ) {
					...
					let exe_chnl = await getChannel((await global.Game.getGuild()), global.Game.Players.get(exe.name).channel_id);
		...
		let ghost_role = await getRole((await global.Game.getGuild()), "Ghosts"),
			...
			player_guild_member = await getGuildMember((await global.Game.getGuild()), global.Game.Players.get(victim_name).id);
	...
	if (await global.Game.getWhichFactionWon(factions)) {
		...
		await global.Game.endGame(game_announce_chnl, interaction);
		return "all"
	}
