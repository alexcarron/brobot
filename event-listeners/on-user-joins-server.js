...
const { getGuild, getRole, addRole, getRoleById } = require("../modules/functions");

const onUserJoinsServer = async function(guildMember) {
	if (guildMember.guild.id === ids.servers.rapid_discord_mafia) {
		const rdmGuild = await getGuild(ids.servers.rapid_discord_mafia);
		...
		const LLGameShowsGuild = await getGuild(ids.servers.ll_game_show_center);
