...
- ~~**Publishing a name removes its characters from your inventory**~~ **(Implemented)**: This is a small tweak that makes publishing feel a little bit more valuable and encourages you to try something different once you have a name you are happy with. Shipped as: publishing removes the published name's characters from inventory and empties the player's current name, for both the paid `/publish-name` command and the unpaid auto-publish that runs at build-phase end. See `docs/mechanics/namesmith-publishing-name-mechanic.md`.
