...
const Announcement = Object.freeze({
	...
	CONGRATULATE_WINNERS: (winning_factions, winning_player_names) => {
		...
		const winning_players_sentence = toListOfWords(bolded_winning_player_names);
		const bolded_winning_factions = winning_factions.map(
			name => `**${name}**`
		);
		const winning_factions_sentence = toListOfWords(bolded_winning_factions);
...
const Feedback = Object.freeze({
	...
	LOOKOUT_SEES_VISITS: (target_player, player_seen_visiting) => {
		...
		return `It seems like **${target_player.name}** was visited by ${toListOfWords(player_names_visiting)} last night.`;
	},
