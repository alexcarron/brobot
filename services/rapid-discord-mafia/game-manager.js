...
const {GameStateManager, Subphase, GameState, PhaseLength} = require("./game-state-manager.js");
const GameDataManager = require("./game-data-manager.js");
...
const { getShuffledArray, getRandomElement } = require("../../utilities/data-structure-utils");
const { fetchChannelsInCategory } = require("../../utilities/discord/category-utils");
...
const { addRoleToMember, fetchRoleByName, removeRoleFromMember } = require("../../utilities/discord/role-utils");
...
class GameManager {
	...
	async resetPlayerOnTrial() {
		...
		if (!this.isMockGame) {
			const rdm_guild = await fetchRDMGuild();
			const on_trial_role = await fetchRoleByName(rdm_guild, RDMDiscordRole.ON_TRIAL);
	...
	async addPlayerToGame(player_name, player_id=undefined, interaction=undefined, isMockUser=false) {
		...
		if (!isMockUser && !this.isMockGame && player_id !== undefined) {
			...
			const
				spectator_role = await fetchRoleByName(rdm_guild, RDMDiscordRole.SPECTATOR),
